...
GATE_MODE="${GATE_MODE:-threshold}"
HEAD_OUTPUT_DIRECTION_TEMPERATURE="${HEAD_OUTPUT_DIRECTION_TEMPERATURE:-0.05}"
PROJECTION_STRENGTHS="${PROJECTION_STRENGTHS:-0,0.25,0.5,0.75,1.0}"
...
HEAD_OUTPUT_PROBE_DIR="${HEAD_OUTPUT_PROBE_DIR:-${BASE_RESULT_PATH}/head_output_direction_probe_${HEAD_OUTPUT_NORMALIZATION}_l${MIN_LAYER}_${MAX_LAYER}_hall${HALLUCINATED_SOURCE}_max${MAX_PER_LABEL}}"
CALIBRATION_NPZ="${CALIBRATION_NPZ:-${HEAD_OUTPUT_PROBE_DIR}/query_direction_calibration.npz}"
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_RESULT_PATH}/head_output_projection_sweep_${HEAD_OUTPUT_NORMALIZATION}_top${DIRECTION_TOP_K}_${GATE_MODE}_max${MAX_PER_LABEL}}"
...
if [ ! -f "${CALIBRATION_NPZ}" ]; then
    echo "[error] missing head-output direction calibration: ${CALIBRATION_NPZ}" >&2
    echo "[hint] run bash_scripts/soft_routing/run_head_output_direction_probe.sh first" >&2
    exit 1
fi
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.soft_routing.query_projection_sweep \
    ...
    --gate-mode "${GATE_MODE}" \
    --query-direction-temperature "${HEAD_OUTPUT_DIRECTION_TEMPERATURE}" \
    2>&1 | tee "${LOG_DIR}/head_output_projection_sweep_${HEAD_OUTPUT_NORMALIZATION}_top${DIRECTION_TOP_K}_${GATE_MODE}_max${MAX_PER_LABEL}.log"

echo "[summary] selected head-output directions"
...
if [ -f "${OUTPUT_DIR}/query_projection_sweep_by_group.csv" ]; then
    column -s, -t "${OUTPUT_DIR}/query_projection_sweep_by_group.csv" | head -40
fi
