...
CALIBRATION_NPZ="${CALIBRATION_NPZ:-${QUERY_PROBE_DIR}/query_direction_calibration.npz}"
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_RESULT_PATH}/query_projection_sweep_${QUERY_NORMALIZATION}_top${DIRECTION_TOP_K}_${GATE_MODE}_max${MAX_PER_LABEL}}"
...
if [ ! -f "${CALIBRATION_NPZ}" ]; then
    echo "[error] missing query direction calibration: ${CALIBRATION_NPZ}" >&2
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.soft_routing.query_projection_sweep \
    ...
    --gate-mode "${GATE_MODE}" \
    --query-direction-temperature "${QUERY_DIRECTION_TEMPERATURE}" \
    2>&1 | tee "${LOG_DIR}/query_projection_sweep_${QUERY_NORMALIZATION}_top${DIRECTION_TOP_K}_${GATE_MODE}_max${MAX_PER_LABEL}.log"

echo "[summary] selected query directions"
