...
WIDE_GATE_GAMMA="${WIDE_GATE_GAMMA:-1.0}"
...
POOL100_PATH="${POOL100_PATH:-${POOL_DIR}/contrastive_top${TOP_POOL_K}.json}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_RESULT_PATH}/wide_pool_gate_top${TOP_POOL_K}_normq${NORM_Q_THRESHOLD}}"
...
if [ ! -f "${BEHAVIORAL_RECORDS}" ]; then
    ...
    MIN_LAYER=13 \
    TOP_K=20 \
    ATTENTION_HEAD_PATH="${POOL100_PATH}" \
...
run_eval() {
    ...
    if [ "${mode}" = "adhh" ]; then
        CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.eval_caption_adhh \
            ...
            --top_k "${top_k}" \
            2>&1 | tee "${LOG_DIR}/wide_pool_${tag}.log"
    else
        CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.eval_caption_adhh \
            --model-path "${MODEL_PATH}" \
            --image-folder "${IMAGE_FOLDER}" \
            ...
            --wide_gate_mode "${mode}" \
            --wide_gate_feature "${feature}" \
            --wide_gate_text_tau "${ADHH_THRESHOLD}" \
            --wide_gate_text_high "${WIDE_GATE_TEXT_HIGH}" \
            --wide_gate_gamma "${WIDE_GATE_GAMMA}" \
...
run_eval "adhh_top20_hard" 20 "adhh" "text"
run_eval "top40_text_hard" 40 "hard" "text"
run_eval "top60_text_hard" 60 "hard" "text"
run_eval "top80_text_hard" 80 "hard" "text"
run_eval "top100_text_hard" 100 "hard" "text"
run_eval "top40_text_norm_hard" 40 "hard" "text_norm"
run_eval "top60_text_norm_hard" 60 "hard" "text_norm"
run_eval "top80_text_norm_hard" 80 "hard" "text_norm"
run_eval "top100_text_norm_hard" 100 "hard" "text_norm"
run_eval "top60_text_norm_continuous" 60 "continuous" "text_norm"
run_eval "top100_text_norm_continuous" 100 "continuous" "text_norm"
...
out = "${OUTPUT_DIR}/wide_pool_gate_summary.csv"
if rows:
...
echo "[summary] wide-pool gate metrics"
if [ -f "${OUTPUT_DIR}/wide_pool_gate_summary.csv" ]; then
    column -s, -t "${OUTPUT_DIR}/wide_pool_gate_summary.csv"
fi
