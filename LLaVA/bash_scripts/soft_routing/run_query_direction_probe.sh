...
SAVE_QUERY_VECTORS="${SAVE_QUERY_VECTORS:-0}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_RESULT_PATH}/query_direction_probe_${QUERY_NORMALIZATION}_l${MIN_LAYER}_${MAX_LAYER}_hall${HALLUCINATED_SOURCE}_max${MAX_PER_LABEL}}"
...
echo "[info] query normalization: ${QUERY_NORMALIZATION}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.soft_routing.build_query_direction_probe \
    ...
    --seed "${SEED}" \
    "${save_vector_args[@]}" \
    2>&1 | tee "${LOG_DIR}/query_direction_probe_${QUERY_NORMALIZATION}_l${MIN_LAYER}_${MAX_LAYER}_hall${HALLUCINATED_SOURCE}_max${MAX_PER_LABEL}.log"

echo "[summary] top query directions"
