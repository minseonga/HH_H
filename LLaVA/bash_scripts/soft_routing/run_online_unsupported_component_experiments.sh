...
UNSUPPORTED_COMPONENT_PREFILL_PROTECT_TOP_K="${UNSUPPORTED_COMPONENT_PREFILL_PROTECT_TOP_K:-0}"
UNSUPPORTED_COMPONENT_RECENT_TEXT_WINDOW="${UNSUPPORTED_COMPONENT_RECENT_TEXT_WINDOW:-8}"
UNSUPPORTED_COMPONENT_SINK_TOP_K="${UNSUPPORTED_COMPONENT_SINK_TOP_K:-0}"
UNSUPPORTED_COMPONENT_SINK_OFFSETS="${UNSUPPORTED_COMPONENT_SINK_OFFSETS:-}"
...
UNSUPPORTED_COMPONENT_QUERY_GATE_TOP_K="${UNSUPPORTED_COMPONENT_QUERY_GATE_TOP_K:-0}"
UNSUPPORTED_COMPONENT_QUERY_GATE_MIN_AUROC="${UNSUPPORTED_COMPONENT_QUERY_GATE_MIN_AUROC:-0.65}"
UNSUPPORTED_COMPONENT_QUERY_GATE_MODE="${UNSUPPORTED_COMPONENT_QUERY_GATE_MODE:-off}"
...
RECORD_TOKEN_SCORE_DIAGNOSTICS="${RECORD_TOKEN_SCORE_DIAGNOSTICS:-0}"
...
echo "[info] prefill protect top-k: ${UNSUPPORTED_COMPONENT_PREFILL_PROTECT_TOP_K}"
echo "[info] recent text window: ${UNSUPPORTED_COMPONENT_RECENT_TEXT_WINDOW}"
echo "[info] sink top-k: ${UNSUPPORTED_COMPONENT_SINK_TOP_K}"
echo "[info] sink offsets: ${UNSUPPORTED_COMPONENT_SINK_OFFSETS:-dynamic}"
echo "[info] all heads: ${UNSUPPORTED_COMPONENT_ALL_HEADS}"
echo "[info] query gate calibration: ${UNSUPPORTED_COMPONENT_QUERY_GATE_CALIBRATION:-none}"
echo "[info] query gate: mode=${UNSUPPORTED_COMPONENT_QUERY_GATE_MODE} top_k=${UNSUPPORTED_COMPONENT_QUERY_GATE_TOP_K} min_auc=${UNSUPPORTED_COMPONENT_QUERY_GATE_MIN_AUROC} aggregation=${UNSUPPORTED_COMPONENT_QUERY_GATE_AGGREGATION}"
echo "[info] query gate shaping: temp=${UNSUPPORTED_COMPONENT_QUERY_GATE_TEMPERATURE} default=${UNSUPPORTED_COMPONENT_QUERY_GATE_DEFAULT} min=${UNSUPPORTED_COMPONENT_QUERY_GATE_MIN} power=${UNSUPPORTED_COMPONENT_QUERY_GATE_POWER}"
...
echo "[info] record token score diagnostics: ${RECORD_TOKEN_SCORE_DIAGNOSTICS}"
...
if [ "${RECORD_UNSUPPORTED_COMPONENT_DIAGNOSTICS}" = "1" ]; then
    ...
    if [ "${RECORD_TOKEN_SCORE_DIAGNOSTICS}" = "1" ]; then
        diagnostic_args+=(--record_token_score_diagnostics)
    fi
fi
...
run_eval() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.eval_caption_adhh \
        ...
        --unsupported_component_prefill_protect_top_k "${UNSUPPORTED_COMPONENT_PREFILL_PROTECT_TOP_K}" \
        --unsupported_component_recent_text_window "${UNSUPPORTED_COMPONENT_RECENT_TEXT_WINDOW}" \
        --unsupported_component_sink_top_k "${UNSUPPORTED_COMPONENT_SINK_TOP_K}" \
        --unsupported_component_sink_offsets "${UNSUPPORTED_COMPONENT_SINK_OFFSETS}" \
        --unsupported_component_query_gate_calibration "${UNSUPPORTED_COMPONENT_QUERY_GATE_CALIBRATION}" \
