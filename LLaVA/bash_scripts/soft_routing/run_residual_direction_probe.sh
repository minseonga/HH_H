...
SAVE_RESIDUAL_VECTORS="${SAVE_RESIDUAL_VECTORS:-0}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${BASE_RESULT_PATH}/residual_direction_probe_${RESIDUAL_NORMALIZATION}_l${MIN_LAYER}_${MAX_LAYER}_hall${HALLUCINATED_SOURCE}_max${MAX_PER_LABEL}}"
...
save_vector_args=()
if [ "${SAVE_RESIDUAL_VECTORS}" = "1" ]; then
...
echo "[info] residual normalization: ${RESIDUAL_NORMALIZATION}"
...
CUDA_VISIBLE_DEVICES="${GPU_ID}" python -m eval_scripts.soft_routing.build_query_direction_probe \
    ...
    --max-layer "${MAX_LAYER}" \
    --surface residual \
    --query-normalization "${RESIDUAL_NORMALIZATION}" \
    --test-fraction "${TEST_FRACTION}" \
    --seed "${SEED}" \
    "${save_vector_args[@]}" \
    2>&1 | tee "${LOG_DIR}/residual_direction_probe_${RESIDUAL_NORMALIZATION}_l${MIN_LAYER}_${MAX_LAYER}_hall${HALLUCINATED_SOURCE}_max${MAX_PER_LABEL}.log"

echo "[summary] top residual directions"
